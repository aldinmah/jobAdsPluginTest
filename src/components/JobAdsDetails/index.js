...
export default function JobAdsDetails(props) {
  const [jobAdDetailsData, setjobAdDetailsData] = useState({})
  const [randomImgURL, setRandomImgURL] = useState(0)
  const [showApplyToJobForm, setShowApplyToJobForm] = useState(false)

  useEffect(() => {
    setjobAdDetailsData(props.selectedJobAd)
    //set random image from image list
    let randomImagePath = JOB_IMAGES_LIST[Math.floor(Math.random() * JOB_IMAGES_LIST.length)]
  ...
  return (
    <Box className="fullRow jobAdsDetailsWrapper">
      <Collapse in={!showApplyToJobForm}>
        ...
        <h1 className="fullRow jobTitle">{jobAdDetailsData?.title}</h1>
        <div className="fullRow locationDurationContainer">
          <div className="floatLeft locationBox">
            ...
            <span className="locationValue">
              {jobAdDetailsData?.municipality?.county || " - "}
            </span>
          </div>
          <div className="floatLeft durationBox">
            ...
            <span className="durationValue">
              {jobAdDetailsData?.employmentDescription || " - "}
            </span>
          </div>
        </div>
        <div className="fullRow applyBtnWrapper">
          <Button
            ...
            onClick={() => {
              if(props.globalConfig?.useInternalApplyForm){
                ...
                window.open(jobAdDetailsData?.applyUrl, "_blank")
              }
              else{
                setShowApplyToJobForm(true)
              }
          ...
          <div className="floatRight socialIconsWrapper">
            <a
              target="_blank"
              rel="noreferrer"
              href={
              ...
              title={"Dela " + jobAdDetailsData?.title + " på Facebook"}
            ...
            <a
              ...
              title={"Dela " + jobAdDetailsData?.title + " på LinkedIn"}
            ...
            <a
              ...
              title={"Dela " + jobAdDetailsData?.title + " på Twitter"}
        ...
        <div className="fullRow desriptionWrapper">
          ...
          {jobAdDetailsData?.description && (
            <div
              className="fullRow descriptionBody"
              dangerouslySetInnerHTML={{
                __html: jobAdDetailsData?.description,
              }}
            ></div>
          )}
          {!jobAdDetailsData?.description && (
            <div className="jobDescriptionEmptyWrapper">
              ...
              {jobAdDetailsData?.service?.name.toLowerCase()} till uppdrag för
              oss hos vår kund. Dina arbetsuppgifter består av sedvanliga
              arbetsuppgifter som konsult inom
              {jobAdDetailsData?.service?.name.toLowerCase()}. Det är
              meriterande om du har erfarenhet inom denna typ av uppdrag och
              bemanning.
            </div>
          )}
        </div>
        ...
        {jobAdDetailsData?.contacts && !!jobAdDetailsData.contacts.length && (
          <div className="fullRow contactInfoWrapper">
            <h2>Contact person(s)</h2>
            <div className="fullRow contactList">
              {jobAdDetailsData.contacts.map((item, index) => {
                return (
                  <div
